...
def test_get_api_rates(api_client_auth):
    response = api_client_auth.get('/api/v1/currency/rates/')
    assert response.status_code == 200
    assert response.json()


def test_post_api_rates_empty(api_client_auth):
    response = api_client_auth.post('/api/v1/currency/rates/')
...
def test_post_api_rates_valid(api_client_auth):
    ...
    response = api_client_auth.post('/api/v1/currency/rates/', data=payload)
